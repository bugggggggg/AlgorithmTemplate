...
44. https://en.wikipedia.org/wiki/Randomized_weighted_majority_algorithm：取一个0<A<1,记录每个人错误次数w[i],则每个人的话语权为A^w[i].每次算出选择其中一个的权重P=Σ选这个的权重/权重和。然后这次有概率P选择这个。
    - 指数减小权重
    - 算出选择一个的权重后，以这个权重为概率进行选择（不一定这个权重大就必选这个）
45. 